...
exports.actualizarEstadoInscripcion = async (req, res) => {
  try {
    ...
    if (curso) {
      ...
      curso.cuposDisponibles = (curso.cuposTotal || curso.cuposTotales) - inscripcionesActivas;
      await curso.save();
    }
...
exports.getEstadisticasCurso = async (req, res) => {
  try {
    ...
    const turnoManana = await Inscription.countDocuments({ 
      courseId: cursoId, turnoPreferido: 'mañana', estado: { $in: ['pendiente', 'confirmado'] }
    });
    const turnoTarde = await Inscription.countDocuments({ 
      courseId: cursoId, turnoPreferido: 'tarde', estado: { $in: ['pendiente', 'confirmado'] }
    });
    const turnoIndistinto = await Inscription.countDocuments({ 
      courseId: cursoId, turnoPreferido: 'indistinto', estado: { $in: ['pendiente', 'confirmado'] }
    });
    ...
    res.json({
      success: true,
      data: {
        totalInscripciones, activos, confirmados, pendientes, cancelados,
        cuposDisponibles: curso ? curso.cuposDisponibles : 0,
        cuposTotal: curso ? curso.cuposTotal : 0,
        porTurno: { manana: turnoManana, tarde: turnoTarde, indistinto: turnoIndistinto }
      }
    });
  } catch (error) {
...
exports.sincronizarCupos = async (req, res) => {
  try {
    ...
    res.json({
      ...
      data: {
        cuposTotal: curso.cuposTotal || curso.cuposTotales,
    ...
    const stats = await Inscription.aggregate([
      { $match: matchQuery },
      {
        $group: {
          ...
          turnoManana: { $sum: { $cond: [{ $eq: ['$turnoPreferido', 'mañana'] }, 1, 0] } },
